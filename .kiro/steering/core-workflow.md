...
## ★ 캐릭터/이미지 작업 타입 구분 규칙 (필수 — 시작 전 무조건 질문)
"캐릭터 만들어줘 / 이미지 만들어줘 / 영상 만들어줘" 류 요청은 목적이 전혀 다른 **두 타입**이 있다.
- **★★ 사용자가 명시적으로 타입을 말하지 않았으면, 어떤 도구도 실행하기 전에(전사·이미지생성·script.ts 등 착수 전에) 반드시 먼저 물어본다.** "이거 ① Remotion 캐릭터 컷(Type A)인가요, ② 캐릭터 포함 이미지 만들고 힉스필드로 영상 제작(Type B)인가요?" — 넘겨짚고 진행하는 것 금지. 롱폼/숏폼 확인과 **동일하게 매 요청마다** 체크한다.
- 판별 힌트로 100% 확실할 때만(예: "script.ts에 넣어줘"=A, "힉스필드로 영상화"=B) 질문 생략 가능. 조금이라도 모호하면 질문한다.
